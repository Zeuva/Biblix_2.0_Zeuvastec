...
function renderMultiLobby() {
  ...
  root.innerHTML = `
    <div class="app">
      ...
      <div class="decks"><div class="decks-label">Cartão da partida (vale para a sala que você criar)</div>${renderDeckSeals()}</div>
      ...
      <div class="panel"><h3>Seu nome</h3><div class="field"><input id="player-name" type="text" maxlength="18" placeholder="Ex: João" value="${Multi.myName || ''}" /><div class="field-hint">Aparecerá para os outros jogadores na sala.</div></div></div>
      <div class="panel"><h3>Criar uma sala nova</h3><p style="font-size:13px;color:var(--parchment-dim);margin:0 0 14px">Cartão selecionado: <b style="color:var(--gold-bright)">${deckLabel(App.deckId)} · ${count}Q</b></p><button class="btn btn-primary btn-block" id="create-room" ${Multi.connecting ? 'disabled' : ''}>${Multi.connecting && Multi.pendingAction === 'create' ? 'Criando sala…' : '👑 Criar sala'}</button></div>
      <div class="panel"><h3>Entrar em uma sala</h3><div class="field"><label>Código da sala</label><input id="room-code" type="text" maxlength="6" placeholder="Ex: A3F9K2" style="text-transform:uppercase;letter-spacing:.1em" /></div><button class="btn btn-secondary btn-block" id="join-room" ${Multi.connecting ? 'disabled' : ''}>${Multi.connecting && Multi.pendingAction === 'join' ? 'Entrando…' : '🔑 Entrar na sala'}</button></div>
...
function renderMultiQuestion(){
  ...
  if(needsFull){
    ...
    wireHeader();
    const nb=document.getElementById('multi-next'); if(nb) nb.addEventListener('click',()=>{ if(Multi.isHost) hostAdvanceManual(); });
    if(Multi.chosen===null){ root.querySelectorAll('#multi-options .option').forEach(btn=>{ btn.addEventListener('click',()=>submitMultiAnswer(Number(btn.dataset.i))); }); }
  } else {
    const pos=document.getElementById('multi-q-pos'); if(pos) pos.textContent=Multi.currentIndex+1;
    const prog=document.getElementById('multi-progress'); if(prog) prog.style.width=(Multi.currentIndex/total*100)+'%';
    updatePlayersListDOM(); updateNextButtonState();
